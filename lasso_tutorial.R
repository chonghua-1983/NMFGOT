...
# metabolites <- load_gene_expr(paste0(current_dir,"/input/example_data/gene_expr_demo_lasso.txt"))
metabolites <- read.table(paste0(current_dir,"/input/metabolites_Lasso.csv"),header=TRUE,  sep=",", row.names=1)
metabolites <- as.matrix(metabolites)
dim(metabolites) #[1]    44 3
# 
# ## load microbiome data (note, here we load microbiome data with sex covariate)
microbes <- read.table(paste0(current_dir,"/input/microbes_Lasso.csv"), header = TRUE, sep=",", row.names=1)
microbes <- as.matrix(microbes)
dim(microbes) #[1]  44 236 -- 235 taxa + 1 sex covariate
# 
# ## Ensure same sampleIDs in both genes and microbes data before sparse CCA
stopifnot(all(rownames(metabolites) == rownames(microbes)))
# 
y <- metabolites #response
...
lasso.df <- data.frame(metabolite = rep(mtb_name, length(lasso.proj.fit$pval)),
...
# 
# 
stabsel.df <- data.frame("metabolite" = mtb_name, "taxa" = taxa.selected)
# if(taxa.selected == "none")
if (length(taxa.selected) == 0)
   stabsel.df$stability_selected = "no"
else
  stabsel.df$stability_selected = "yes"
# 
head(stabsel.df)
# 
# ################ Merge output of lasso+hdi and stabsel #################
# 
overlap_lasso_stabsel <- merge(lasso.df,stabsel.df, by = c("metabolite","taxa"))
head(overlap_lasso_stabsel)
merged_df <- rbind(merged_df,overlap_lasso_stabsel)

}

write.csv(merged_df,"overlap_lasso_stabsel_ESRDtest100.csv")
